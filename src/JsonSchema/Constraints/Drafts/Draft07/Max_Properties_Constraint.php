...
class Max_Properties_Constraint implements Constraint_Interface
{
    use Error_Bag_Proxy;
    ...
    public function check(&$value, $schema = null, ?Json_Pointer $path = null, $i = null): void
    {
        ...
        if ($count <= $schema->max_properties) {
            return;
        }
        $this->add_error(Constraint_Error::PROPERTIES_MAX(), $path, ['maxProperties' => $schema->max_properties, 'found' => $count]);
    }
}